...
# `ollama list` always reports a tag — normalize a tagless override so the
# stale-model sweep below doesn't delete and re-pull the model on every boot.
case "$MODEL" in
  *:*) ;;
  *) MODEL="${MODEL}:latest" ;;
esac
...
# Forward platform stop signals to the server instead of letting PID 1
# swallow them (otherwise ollama is hard-killed when the dyno stops).
trap 'kill "$SERVER_PID" 2>/dev/null' TERM INT

# Wait for the API to come up. The base image ships no curl — probe via the
# CLI, which talks to OLLAMA_HOST.
...
# Pull the model if we don't already have it. -Fx: fixed-string whole-line
# match — the model name's dots would otherwise act as regex wildcards.
